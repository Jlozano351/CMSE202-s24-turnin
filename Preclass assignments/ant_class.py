...
class Ant:
    ...
    head_home()
        ...
        It randomly chooses to either go down or to the left and as
        home is in the bottom left corner and it cannot go out of bounds
        it will eventually end at home.

    move()
        moves the ant in one of many ways (head_home() or search_for_food())

    search for food()
        moves the ant in a random direction if no pheromones are present, if 
        pheromones are present it moves the ant there.
...
def run(num_ants=100, x_dim=70, y_dim=30):
    ...
    for i in range(0, 100):
        ...
        # plot world
        plt.imshow(smell.T, origin='lower', aspect='equal', cmap="inferno", vmin=vmin, vmax=vmax)
        plt.colorbar(label='Pheromones') 
